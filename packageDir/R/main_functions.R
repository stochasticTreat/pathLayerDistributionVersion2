...
test.multiRunFromSettings<-function(){
	
	incFname = "./input/test_files/somatic_mutation_aberration_summary_incramented_settings.txt"
	incSettingsTable = read.table(file=incFname, sep="\t", header=T, stringsAsFactors=F)
	incSettingsTable = checkForceRowNames(tab=incSettingsTable)
	...
	scr1 = study_res@results$settings_comparison_results$"1"
	scr2 = study_res@results$settings_comparison_results$"2"
	
	checkException(expr=checkEquals(target=scr1$somatic_mutation_aberration_summary$pathsummary, 
														 current=scr2$somatic_mutation_aberration_summary$pathsummary),
								 msg="Exception successfully being thrown indicates changing the settings will change the output")
	checkEquals(target=scr1$somatic_mutation_aberration_summary$patientsums, current=scr2$somatic_mutation_aberration_summary$patientsums)
	
	checkException(expr=checkEquals(target=scr1$overlap_analysis$"Aberrationally enriched, containing drug targets", 
																	current=scr2$overlap_analysis$"Aberrationally enriched, containing drug targets"),
								 msg="Exception successfully being thrown indicates changing the settings will change the output")
...
multiRunFromSettings<-function(study, incSettingsTable, incArmName){
	
	#first, figure out which arms aren't the inc arm
	allanalyses = names(study@studyMetaData@settings)	
	analyses = allanalyses[!grepl(pattern="^default|^overlap_analysis", x=allanalyses, ignore.case=T)]#remove any settings starting with the string "default"
	analyses = setdiff(analyses, incArmName)#remove the incramented settigns
	if(!length(analyses)){
		tmp = readline("Sorry, no settings could be found.\nSelect 2 from main menu to load settings from another study.\nPress enter to continue. ")
		return(study)
	}
	
	cat("Normal settings being run for these intput arms:\n")
	cat(analyses, "\n")
	for(a in analyses){
		study = runArm(armDescription=a, study=study, fromDescription=F, interactive=F)
	}
	
	normalSettings = study@studyMetaData@settings
	normalResults = study@results
	
	incRes = list()
	#now incrament through the inc settings
	for(i in 1:ncol(incSettingsTable)){
		#pull out the current column of settings
		cs = incSettingsTable[,i,drop=F]
		...
		incRes[[i]] = study@results
	}

	names(incRes)<-as.character(1:length(incRes))
	study@results = normalResults
	study@results[["settings_comparison_results"]] = incRes
	
	study@studyMetaData@settings = normalSettings
